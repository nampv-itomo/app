...
import playIcon from "../../access/icons/playing.gif";
...
const SongItem = ({ song, index, selectSong, selectedSongId, playerState }) => {
  ...
  const selector = () => {
    return (
      <a draggable="false" href={song.url}>
        <svg role="img" height="24" width="24" viewBox="0 0 24 24" className="download-link">
          <path
  ...
  const phaser = () => {
    if (selectedSongId === song.id && playerState) {
      return (
        <div className="index">
          <img alt="" src={playIcon} id="focused" className="small-icon" />
        </div>
      );
    } else {
